...
export function InterfacesLogoSquare() {
  return (
...
export function BrandBadge() {
  ...
  return (
    <div 
      className="relative shrink-0 w-full mb-4 cursor-pointer group"
      onClick={() => navigate("/")}
    >
      <div className="flex items-center p-1 w-full rounded-xl transition-colors hover:bg-white/5">
        <div className="h-10 w-8 flex items-center justify-center pl-2">
          <InterfacesLogoSquare />
        </div>
        <div className="px-2 py-1">
          <div className="font-semibold text-[18px] text-white tracking-widest uppercase">
            ContextSwitch
          </div>
        </div>
      </div>
...
export function AvatarCircle() {
  return (
    <div className="relative rounded-full shrink-0 size-8 bg-neutral-900">
...
export function SearchContainer({ isCollapsed = false, value, onChange }: { isCollapsed?: boolean, value: string, onChange: (v: string) => void }) {
  return (
    <div
      className={`relative shrink-0 transition-all mb-4 duration-500 ${
        isCollapsed ? "w-full flex justify-center" : "w-full"
      }`}
      style={{ transitionTimingFunction: softSpringEasing }}
    >
      <div
        className={`bg-white/5 backdrop-blur-md h-10 relative rounded-xl flex items-center transition-all duration-500 border border-white/10 ${
          ...
          <div className="flex flex-col justify-center size-full">
            <div className="flex flex-col gap-2 items-start justify-center pr-2 py-1 w-full">
              <input
                ...
                className="w-full bg-transparent border-none outline-none text-[14px] text-white placeholder:text-neutral-500 leading-[20px]"
...
export function IconNavigation({
  ...
  const navItems = [
    ...
    { id: "insights", icon: <Analytics size={18} />, label: "Insights" },
  ];

  return (
    <aside className="bg-transparent flex flex-col gap-4 items-center p-4 w-20 min-w-20 border-r border-white/5">
      <button 
        onClick={() => navigate("/")}
        className="mb-4 size-10 flex items-center justify-center rounded-xl transition-colors hover:bg-white/5"
        title="Go to Home"
      >
      ...
      <div className="flex flex-col gap-3 w-full items-center">
        ...
        <button 
          ...
          className={`size-10 flex items-center justify-center rounded-xl transition-all duration-500 hover:bg-white/5 group ${activeSection === 'account' ? 'bg-white/10' : ''}`}
          title="Account"
        >
          <AvatarCircle />
        </button>
...
export function SectionTitle({
  ...
  if (isCollapsed) {
    return (
      <div className="w-full flex justify-center transition-all duration-500 mb-6" style={{ transitionTimingFunction: softSpringEasing }}>
        <button
          type="button"
          onClick={onToggleCollapse}
          className="flex items-center justify-center rounded-xl size-10 min-w-10 transition-all duration-500 hover:bg-white/5 text-neutral-400 hover:text-white"
          style={{ transitionTimingFunction: softSpringEasing }}
        >
  ...
  return (
    <div className="w-full overflow-hidden transition-all duration-500 mb-6" style={{ transitionTimingFunction: softSpringEasing }}>
      <div className="flex items-center justify-between">
        ...
        <div className="pr-1 flex items-center gap-2">
          <button
            type="button"
            onClick={onToggleCollapse}
            className="flex items-center justify-center rounded-xl size-8 min-w-8 transition-all duration-500 hover:bg-white/5 text-neutral-400 hover:text-white"
            style={{ transitionTimingFunction: softSpringEasing }}
          >
...
export function MenuItem({
  ...
  return (
    ...
    >
      <div
        className={`rounded-xl cursor-pointer transition-all duration-500 flex items-center relative group ${
          item.isActive ? "bg-white/10 shadow-[inset_0_0_0_1px_rgba(255,255,255,0.1)]" : "hover:bg-white/5"
        } ${isCollapsed ? "w-10 min-w-10 h-10 justify-center p-4" : "w-full min-h-[44px] px-3 py-2"}`}
        style={{ transitionTimingFunction: softSpringEasing }}
        onClick={handleClick}
        title={isCollapsed && typeof item.label === 'string' ? item.label : undefined}
      >
        {item.icon && <div className={`flex items-center justify-center shrink-0 ${item.isActive ? 'text-accent' : 'text-neutral-400 group-hover:text-white'}`}>{item.icon}</div>}
        ...
        <div
          ...
          style={{ transitionTimingFunction: softSpringEasing }}
        >
          <div className={`text-[14px] leading-[20px] font-medium ${item.isActive ? 'text-white' : 'text-neutral-300'}`}>
            {item.label}
          </div>
        </div>

        {!isCollapsed && item.rightElement && (
    ...
    <div className="flex flex-col w-full mb-4">
      ...
      >
        <div className="flex items-center h-full px-4">
          <div className="text-[12px] font-semibold text-neutral-500 tracking-wider uppercase">
            {section.title}
          </div>
        </div>
      </div>
...
export function DetailSidebar({
  ...
  return (
    <aside
      className={`bg-white/5 backdrop-blur-3xl border-r border-white/5 flex flex-col items-start p-4 transition-all duration-500 min-h-screen ${
        isCollapsed ? "w-20 min-w-20 !px-0 justify-start pt-4" : "w-[340px] min-w-[340px]"
      }`}
      style={{ transitionTimingFunction: softSpringEasing }}
    >
      {!isCollapsed && <BrandBadge />}

      <SectionTitle title={title} rightElement={rightElement} onToggleCollapse={toggleCollapse} isCollapsed={isCollapsed} />
      
      {!isCollapsed && <SearchContainer isCollapsed={isCollapsed} value={search} onChange={onSearchChange} />}

      <div
        className={`flex flex-col w-full overflow-y-auto overflow-x-hidden transition-all duration-500 flex-1 hide-scrollbar ${
